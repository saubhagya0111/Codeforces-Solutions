...
void solution()
{
    ...
    in(n);
    vi v;
    //Storing the growths of each month in the a vector
    incr_loop(0, 12)
    {
        in(element);
        v.pb(element);
    }
    //Sorting the elements in descending order so that he can produce maximum out put in a given time
    sort(all(v), greater<int>());
    i = 0;
    ll sum = 0;
    //Iterating over the vector till the desired growth is not achieved
