...
#endif
/************************************/
//If the input is R then print the one index before the given letter X
//If the input is L then print the one index after the given letter X
...
void solution()
{
    ...
    string s1;
    in(s1);
    incr_loop(0, s1.length())
    {
        if (ch == 'L')
        {
            s1[i] = s[return_index(s, s1[i]) + 1];
        }
        else if (ch == 'R')
        {
            s1[i] = s[return_index(s, s1[i]) - 1];
